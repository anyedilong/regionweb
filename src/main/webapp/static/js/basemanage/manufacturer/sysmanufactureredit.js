...
$(function(){
    ...
    function initData(id){
        //查询数据，绑定到元素
        var showUrl = roadPath + '/family/manufacturer/show';
        var json = {'id':id}
        $.ajax({
            type: 'POST',
            async: false,
    ...
    //保存form表单
    function saveForm(){
    	if(initValidate("生产厂商名称", $("#name").val()) && initValidate("生产厂商编号", $("#code").val()) &&
    			initValidate("地址", $("#address").val()) &&initValidate("联系方式", $("#phone").val())){
    		var saveUrl = roadPath + '/family/manufacturer/save';
